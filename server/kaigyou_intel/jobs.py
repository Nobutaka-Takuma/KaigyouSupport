...
#: 周辺一般（プレDD レポート）の段。
#:
#: **2 段です。** 以前は 4 段——統計から特徴を抽出し、問いを立てて Web で調べ、
#: 需要形成を組み立て、レポートに書き直す——でしたが、仮説を立てて検証する
#: 筋書きがそのまま本文になり、読み手が知りたい「この商圏はどうなのか」が
#: どこにも無い文書になっていました。
#:
#: いまは 静的データは DB から数えるだけ（1 段目・API 費用ゼロ）、LLM は
#: それを読んで書くだけ（2 段目）です。
STEP_NAMES = {
    1: "事実の確定",
    2: "プレDDレポート執筆",
    # **第II部。ここだけが推論する段です。** 第I部は事実だけ、第II部は
    # 「ここで開業するならどうするか」。混ぜると第I部が信用を失います。
    3: "提言レポート執筆",
}
...
#: 旧・4段の探索型（analysis_kind = research）。**消していません。**
#: 既定からは外しましたが、問いと仮説の筋書きを見たいときに残してあります。
RESEARCH_STEP_NAMES = {
    1: "商圏特徴抽出",
    2: "外部コンテクスト調査",
    3: "需要形成・患者分析と経営判断",
