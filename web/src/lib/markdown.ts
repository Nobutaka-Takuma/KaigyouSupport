 ...
 * 見出し・強調・箇条書き・表・引用・短いコード・裸の URL の 7 つしか
 * ありません。汎用のパーサを入れると、扱わない記法のために依存が 2 つ
 * 増えます（パーサ本体と消毒器）。このアプリの実行時依存は 4 つで、そこに釣り合いません。
 *
 ...
 *
 * 対応していない記法（コードブロック・画像・`[題名](URL)` 形式のリンク）は、
...
/** 行の中の記法。`**強調**`、`` `印` ``、裸の URL の 3 つ。 */
function inline(raw: string): string {
  const escaped = escapeHtml(raw);
  const bolded = escaped.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>");
  // 提言（第II部）は推論の段に `FACT` `HYPOTHESIS` のような印を付けます。
  // 素通しにすると、本文にバッククォートが文字のまま並びます。
