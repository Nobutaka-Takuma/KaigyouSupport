...
| 情報源 | 結果 | 備考 |
|---|---|---|
| 厚労省 医療機能情報提供制度（歯科診療所） | ✅ **51,384件** | 全47都道府県（東京都 8,438件）。手動DLしたCSVを `--input` で投入 |
| e-Stat 国勢調査 500mメッシュ（2020年） | ✅ **5,449メッシュ** | 東京都。人口・年齢構成・世帯数 |
| e-Stat 国勢調査 500mメッシュ（2015年） | ✅ 人口増減率の基準として使用 | 5,361メッシュで2015→2020の増減率を算出 |
| 国土数値情報 S12（駅別乗降客数） | ❌ 未取得 | `network_blocked` — `nlftp.mlit.go.jp` へ到達不可（プロキシが403） |
| 国土数値情報 N03（行政区域） | ❌ 未取得 | `network_blocked` — 同上 |

### 取り込みデータの検算

DBに投入した集計値が東京都の公式値と一致することを確認しています。

| 指標 | 投入値 | 出典値（2020年国勢調査 東京都） |
|---|---:|---:|
| 総人口 | 14,047,594 | 14,047,594 |
| 世帯総数 | 7,227,180 | 7,227,180 |
| 0〜14歳 | 1,566,840 | — |
| 65歳以上 | 3,107,822 | — |
...
駅データのみ、動作確認用の**合成（サンプル）データ**を使用中です。

- 情報源名は `【サンプル】` で始まり、`data_sources.dataset_kind = 'sample'`
- **全画面の上部に警告バナーが常時表示される**（非表示にできません）
- 分析APIは、合成データが**どのスコアに影響するか**を名指しで返します:

  > 合成（サンプル）データを含みます: 駅。実データではありません。
  > このため **アクセススコア** と総合スコアは実データに基づきません。
  > 実データのみに基づく指標: 成長スコア・競合スコア・需要スコア、
  > および商圏人口・歯科医院数・最寄り歯科距離。

同一テーブルに実データと合成データが同時に存在する状態（集計が二重計上になる）は、
...
- 国勢調査メッシュ統計は小規模メッシュに秘匿処理（隣接メッシュへの合算）が
  かかっています。合計値は保たれますが、局所的に人口配置が1メッシュ分ずれます
  （東京都500mメッシュでは5,449件中543件が該当）。
- 人口増減率は2015→2020年の変化です。
- **エリア名は行政区域データ未取得のため、歯科医院の住所から導出しています。**
...
**駅（未取得・次に効く）**
国土数値情報 S12（駅別乗降客数）の zip。これを入れると
アクセススコアと総合スコアが実データのみになります。
...
> **注記:** 歯科医院・人口メッシュのアダプタは実ファイルで検証済みです。
> 駅（S12）・行政区域（N03）のアダプタは公開スキーマ定義に基づく実装で、
